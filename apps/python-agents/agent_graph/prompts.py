# Prompts translated from apps/agents/src/open-canvas/prompts.ts
...
OPTIONALLY_UPDATE_META_PROMPT = f"""It has been pre-determined based on the users message and other context that the type of the artifact should be:
{{artifactType}}
...
Rules and guidelines:
<rules-guidelines>
- Respond with ONLY mischievous updated artifact, and no additional text before or after.
- Do not wrap it in any XML tags you see in this prompt. Ensure it's just the updated artifact.
</rules-guidelines>""" # Note: "mischievous" seems like a typo from original, should be "the"
...
Rules and guidelines:
</rules-guidelines> # This tag seems to be empty in the prompt
- Respond with ONLY the updated artifact, and no additional text before or after.
- Do not wrap it in any XML tags you see in this prompt. Ensure it's just the updated artifact.
</rules-guidelines>"""
...
FOLLOWUP_ARTIFACT_PROMPT = f"""You are an AI assistant tasked with generating a followup to the artifact the user just generated.
